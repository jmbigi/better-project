...
## Calibracion (2026-09-19, set v8 de 148 casos)

Se anadio `scripts/jev_calibration.py` y `JEV_TEMPERATURE` al motor. El set
`.docs/knowledge/ai/jev_calibration_set.json` (v8, 148 casos: 52 noul, 48 choice,
48 score) se basa en P0/P1, el estado de los REQ y las lecciones LSN; sus
etiquetas fueron **revisadas y aprobadas** por el programador (lotes 2-7;
P1.15).
...
Resultado medido (Qwen3.5-4B Q4_K_M, 148 casos):

| Metrica | T=1 | T=1.6 | CV antes | CV despues |
|---|---|---|---|---|
| NLL | 0.765 | 0.732 | 0.765 | 0.733 |
| Brier | 0.450 | 0.434 | 0.450 | 0.435 |
| ECE | 0.106 | 0.095 | 0.178 | 0.166 |
| Accuracy | 0.662 | 0.662 | 0.662 | 0.662 |

Accuracy por tipo (T=1.6): choice 0.896 [0.778, 0.955] (n=48), noul 0.615
[0.480, 0.735] (n=52), score 0.479 [0.345, 0.617] (n=48); global 0.662
[0.583, 0.733]. Informe JSON (generado, no versionado) en
...
| `python3 scripts/jev_calibration_merge.py --aplicar` (REQ-018) | set v8, 148 casos (52 noul / 48 choice / 48 score) |
| `.venv/bin/python scripts/jev_calibration.py --write` (v8, 148 casos) | T=1.6; acc 0.662 [0.583,0.733]; choice 0.896, noul 0.615, score 0.479 |
...
1. **Fusionar el lote 8 de candidatos**: revisar con
   `python3 scripts/jev_review.py --calibracion` y fusionar con
   `scripts/jev_calibration_merge.py --aplicar` (18 candidatos N53-N58,
   C49-C54, S49-S54). Objetivo a medio plazo: >=100 casos por tipo.

El programador confirmo el 2026-09-19 las clasificaciones de REQ-012
presentadas y los lotes 2 a 7 de candidatos (40 -> 148 casos, v8). La
calibracion se re-ejecuto con el set v8: T recomendada `1.6`, accuracy global
0.662 [0.583, 0.733]. Mejora anadida: cache de predicciones que acelera las
re-calibraciones (solo computa los casos nuevos).
